...
class SequenceDataset(Dataset):
    ...
    def _get_start_indices(self) -> List[int]:
        """シーケンスの開始インデックスを取得"""
        max_start_idx = len(self.data_files) - self.sequence_length + 1
        indices = []

        # streaming=True の動作: sequence_length ごとにシーケンスの開始位置を設定
        step = self.sequence_length
        for idx in range(0, max_start_idx, step):
            if self.guarantee_label:
                # シーケンス内に少なくとも1つのラベルがあるか確認
                if any(self._has_label(i) for i in range(idx, idx + self.sequence_length)):
                    indices.append(idx)
            else:
                indices.append(idx)
